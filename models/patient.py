...
class Patient:
    def __init__(self, name, cnp, severity):
        # initializam pacientul cu informatiile de baza
        self.name = name  
        self.cnp = cnp    
    ...
    def composite_priority(self):
        # calculeaza prioritatea compozita folosind formula: severitate * 10 - (minute_asteptare / 30)
        minutes = (dt.now() - self.arrival_time).total_seconds()/60
        return self.severity*10 - (minutes / 30)
    
    def __str__(self):
        # afiseaza informatii utile despre pacient pentru useri
